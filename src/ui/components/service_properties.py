...
class ServicePropertiesWindow(ctk.CTkToplevel):
    def __init__(self, master_tab, servicos_view, service_name, live_states=None):
        ...
        self.title(f"Propriedades Enterprise: {service_name}")
        ...
        # Variáveis de Manutenção (Snooze)
        snooze_cfg = self.cfg.get("snooze", {"active": False, "until": 0})
        self.snooze_active = snooze_cfg.get("active", False)
        self.snooze_until = snooze_cfg.get("until", 0)
    ...
    def _inicializar_chaves_padrao(self):
        """Garante que todas as chaves do Watchdog existam no dicionário"""
        defaults = {
            ...
            "emergency": {"max_retries": 3, "reset_days": 0, "run_script": ""},
    ...
    def _create_section(self, parent, title, help_msg=None):
        ...
        if help_msg:
            btn_help = ctk.CTkButton(hdr_frm, text="?", width=20, height=20, corner_radius=10, fg_color=AppColors.DUSK_BLUE, hover_color=AppColors.CHARCOAL_BLUE, command=lambda: self._show_help(f"Ajuda: {title}", help_msg))
    ...
    def _build_top_tabs(self):
        ...
        self.btn_abas = {}
        for txt, cmd in abas:
            btn = ctk.CTkButton(self.tabs_frame, text=txt, fg_color="transparent", text_color=AppColors.CHARCOAL_BLUE, hover_color=AppColors.PLATINUM, font=("Arial", 12, "bold"), height=30, corner_radius=0, command=cmd)
    ...
    def _build_geral_tab(self):
        ...
        grid_frm.pack(fill="x", padx=15, pady=10)
        grid_frm.grid_columnconfigure(0, weight=0, minsize=180)
        ...
        ctk.CTkLabel(grid_frm, text="Caminho do Executável:", font=("Arial", 12, "bold"), anchor="nw").grid(row=3, column=0, sticky="nw", pady=5)
        path_box = ctk.CTkTextbox(grid_frm, height=40, fg_color="transparent", text_color=AppColors.NIGHT)
        ...
        ctk.CTkComboBox(grid_frm, values=["Automático", "Automático (Atraso na inicialização)", "Manual", "Desativado"], variable=self.startup_var, width=250, fg_color=AppColors.WHITE, text_color=AppColors.NIGHT).grid(row=4, column=1, sticky="w", pady=5)
        ...
        self.btn_start = ctk.CTkButton(ctrl_frame, text="▶ Iniciar", fg_color="#28a745", hover_color="#218838", width=100, state="disabled" if is_run else "normal", command=lambda: self._cmd("start"))
        self.btn_start.pack(side="left", padx=5)
        self.btn_stop = ctk.CTkButton(ctrl_frame, text="⏹ Parar", fg_color="#dc3545", hover_color="#c82333", width=100, state="normal" if is_run else "disabled", command=lambda: self._cmd("stop"))
        self.btn_stop.pack(side="left", padx=5)
        self.btn_pause = ctk.CTkButton(ctrl_frame, text="⏸ Pausar", fg_color="#ffc107", hover_color="#e0a800", text_color="black", width=100, state="normal" if is_run else "disabled", command=lambda: self._cmd("pause"))
        self.btn_pause.pack(side="left", padx=5)
        self.btn_resume = ctk.CTkButton(ctrl_frame, text="⏯ Continuar", fg_color="#17a2b8", hover_color="#138496", width=100, state="disabled", command=lambda: self._cmd("continue"))
        self.btn_resume.pack(side="left", padx=5)

        # MODO DE MANUTENÇÃO (SNOOZE)
        msg_snooze = "Pausa temporariamente todas as automações do Watchdog.\nÚtil quando um técnico precisa atualizar o sistema ou parar o serviço manualmente."
        sec_snooze = self._create_section(frm, "Modo de Manutenção (Snooze)", msg_snooze)
        
        row_snz = ctk.CTkFrame(sec_snooze, fg_color="transparent")
        row_snz.pack(fill="x", padx=15, pady=10)
        ctk.CTkLabel(row_snz, text="Pausar o Watchdog para este serviço por:", text_color=AppColors.NIGHT).pack(side="left", padx=(0, 10))
        self.cb_snooze = ctk.CTkComboBox(row_snz, values=["15 Minutos", "30 Minutos", "1 Hora", "12 Horas", "Indeterminado"], width=150, fg_color=AppColors.WHITE, text_color=AppColors.NIGHT)
        self.cb_snooze.set("30 Minutos")
        self.cb_snooze.pack(side="left", padx=10)
        self.btn_snooze = ctk.CTkButton(row_snz, text="Ativar Pausa", fg_color="#ffc107", text_color="black", hover_color="#e0a800", command=self._toggle_snooze)
        self.btn_snooze.pack(side="left", padx=10)

        self.lbl_snooze_status = ctk.CTkLabel(sec_snooze, text="", font=("Arial", 11, "italic"))
        self.lbl_snooze_status.pack(padx=15, pady=(0, 10), anchor="w")
        self._update_snooze_ui()

        # Ações do Watchdog (Reset)
        ...
        ctk.CTkButton(act_frame, text="🔄 Zerar Contadores de Falha", fg_color=AppColors.TRANSPARENT, border_color=AppColors.FLAG_RED, border_width=2, text_color=AppColors.FLAG_RED, hover_color="#ffe6e6", command=self._reset_counters).pack(side="left", padx=(0, 10))
        ctk.CTkButton(act_frame, text="🗑️ Resetar Todas as Configurações", fg_color=AppColors.TRANSPARENT, border_color=AppColors.CHARCOAL_BLUE, border_width=2, text_color=AppColors.CHARCOAL_BLUE, hover_color=AppColors.PLATINUM, command=self._reset_configs).pack(side="left")
    ...
    def _toggle_snooze(self):
        if self.snooze_active:
            ...
            val = self.cb_snooze.get()
            if val == "15 Minutos": self.snooze_until = time.time() + (15 * 60)
            elif val == "30 Minutos": self.snooze_until = time.time() + (30 * 60)
            elif val == "1 Hora": self.snooze_until = time.time() + (60 * 60)
            elif val == "12 Horas": self.snooze_until = time.time() + (12 * 60 * 60)
            elif val == "Indeterminado": self.snooze_until = -1
        self._update_snooze_ui()

    def _update_snooze_ui(self):
        if self.snooze_active:
            self.btn_snooze.configure(text="Retomar Monitoramento", fg_color="#28a745", text_color="white", hover_color="#218838")
            if self.snooze_until == -1:
                self.lbl_snooze_status.configure(text="⚠️ O Watchdog está pausado por tempo indeterminado.", text_color="#d39e00")
            else:
                dt = datetime.fromtimestamp(self.snooze_until).strftime("%H:%M:%S")
                self.lbl_snooze_status.configure(text=f"⚠️ O Watchdog está pausado e retornará às {dt}.", text_color="#d39e00")
        else:
            self.btn_snooze.configure(text="Ativar Pausa", fg_color="#ffc107", text_color="black", hover_color="#e0a800")
            self.lbl_snooze_status.configure(text="✅ Monitoramento Ativo. O Watchdog está vigiando este serviço.", text_color="green")
    ...
    def _build_recuperacao_tab(self):
        ...
        for s_eng, s_pt in status_possiveis:
            ...
            cb = ctk.CTkComboBox(row, values=opcoes_seguras[s_eng], fg_color=AppColors.WHITE, text_color=AppColors.NIGHT)
        ...
        msg_trans = "Se o tempo definido aqui esgotar (status Iniciando/Parando travado), o Watchdog fará um Kill Forçado no processo."
        ...
        ctk.CTkCheckBox(row_pend_en, text="Habilitar Verificação de Tempo de Transição", variable=self.chk_pend_var, text_color=AppColors.NIGHT, fg_color=AppColors.DUSK_BLUE).pack(side="left")
        ...
        ctk.CTkButton(row_emg, text="📁 Procurar...", width=80, fg_color=AppColors.DUSK_BLUE, command=self._procurar_script).pack(side="right")
        ...
        ctk.CTkCheckBox(row_hb_en, text="Ativar reinicialização por inatividade (CPU 0%)", variable=self.chk_hb_var, text_color=AppColors.NIGHT, fg_color=AppColors.DUSK_BLUE).pack(side="left")
    ...
    def _build_deps_tab(self):
        ...
        ctk.CTkLabel(row_beh, text="Comportamento do Watchdog:", width=260, anchor="w", text_color=AppColors.NIGHT).pack(side="left")
        
        opcoes_comp = ["Não Fazer Nada", "Ordem de Inicialização", "Efeito Dominó (Queda)"]
        self.cb_dep_behavior = ctk.CTkComboBox(row_beh, values=opcoes_comp, command=self._update_dep_desc, fg_color=AppColors.WHITE, text_color=AppColors.NIGHT)
    ...
    def _update_dep_desc(self, choice):
        if choice == "Não Fazer Nada": txt = "ℹ️ O sistema apenas regista a dependência, mas não toma nenhuma ação."
        elif choice == "Ordem de Inicialização": txt = "🚀 Antes de iniciar este serviço, o Watchdog ligará o serviço acima caso ele esteja parado."
        else: txt = "⚠️ Efeito Dominó: Se o serviço acima cair, este serviço será PARADO IMEDIATAMENTE para evitar erros, e religado quando o outro voltar."
    ...
    def _build_agendamento_tab(self):
        ...
        ctk.CTkCheckBox(row_en, text="Ativar Reinício Preventivo", variable=self.chk_sched_var, fg_color=AppColors.DUSK_BLUE, text_color=AppColors.NIGHT).pack(side="left")
        ...
        self.cb_days = ctk.CTkComboBox(row_days, values=dias, fg_color=AppColors.WHITE, text_color=AppColors.NIGHT)
    ...
    def _build_footer(self):
        ...
        ctk.CTkButton(footer, text="Guardar Configurações", text_color=AppColors.WHITE, fg_color=AppColors.DUSK_BLUE, corner_radius=4, height=34, font=("Arial", 13, "bold"), command=self._salvar_alteracoes).pack(side="right", padx=5)
        ctk.CTkButton(footer, text="Cancelar", text_color=AppColors.CHARCOAL_BLUE, fg_color=AppColors.TRANSPARENT, hover_color=AppColors.PLATINUM, height=34, command=self.destroy).pack(side="right", padx=5)
    ...
    def _salvar_alteracoes(self):
        ...
        if act_running == "Reiniciar":
            msg = ("⚠️ EFEITO LOOP DETECTADO!\n\nVocê configurou a ação 'Reiniciar' para o status 'Em Execução'. "
                   "Isso fará o Watchdog reiniciar o serviço infinitamente.\n\n"
        ...
        try:
            ...
            self.cfg["pending_timeouts"] = {"enabled": self.chk_pend_var.get(), "start": int(self.ent_pend_start.get()), "stop_others": int(self.ent_pend_stop.get())}
            
            self.cfg["emergency"] = {"max_retries": int(self.ent_retries.get()), "reset_days": int(self.ent_em_reset.get()), "run_script": self.ent_script.get().strip()}
            
            self.cfg["perf_limits"] = {
                "cpu": float(self.ent_cpu.get()), "ram": float(self.ent_ram.get()), "disk": float(self.ent_disk.get()),
                "tolerance": int(self.ent_tol.get()), "max_restarts": int(self.ent_perf_max.get()), "reset_days": int(self.ent_perf_reset.get())
            }
            
            self.cfg["heartbeat"] = {
                "enabled": self.chk_hb_var.get(), "timeout": int(self.ent_hb_time.get()),
                "max_restarts": int(self.ent_hb_max.get()), "reset_days": int(self.ent_hb_reset.get())
            }
            
            self.cfg["snooze"] = {"active": self.snooze_active, "until": self.snooze_until}
            self.cfg["custom_deps"] = {"services": self.ent_deps.get().strip(), "behavior": self.cb_dep_behavior.get()}
            self.cfg["schedule"] = {"enabled": self.chk_sched_var.get(), "days": self.cb_days.get(), "time": formatted_time}
